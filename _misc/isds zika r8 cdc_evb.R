...
dates = paste("'", beginDate,"' AND '", endDate, "'", sep = "") #'beginDate' AND 'endDate'
...
#format date fields    ----- Harold, I think these commented out sections can be deleted; test first though.
#result <- tryCatch({
...
df.DISEASE_processed <- aggregate(df.disease, by=list(df.disease$Unique_Visiting_ID), new_function(x))
